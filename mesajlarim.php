...
include "header.php";
 ...
 $sorguGelenMesaj = "SELECT * FROM kullanicilar_mesaj WHERE k_msj_kime='".$kayitKime['id']."' GROUP BY k_msj_kimden ORDER BY k_msj_id DESC";
...
<div class="row mt-4">
 ...
 <div class='row justify-content-center'>
	...
	?>
		...
		<td><?php
			$sorguMsjKimdenAdSoyad = "SELECT adsoyad FROM kullanicilar WHERE id='".$kayitGelenMesaj['k_msj_kimden']."' LIMIT 0,1";
		...
		if($sayMsjIcerik>0){
			...
			echo "<a href='msj_detay_gonderilen.php?id=".$kayitMsjIcerik['k_msj_id']."' class='btn btn-info m-r-1em'> <span
			class='glyphicon glyphicon glyphicon-eye-open'></span> Detay</a>";
		}else{
			// kayıt detay sayfa bağlantısı
		 echo "<a href='msj_detay.php?id=".$kayitGelenMesaj['k_msj_id']."' class='btn btn-info m-r-1em'> <span
		 class='glyphicon glyphicon glyphicon-eye-open'></span> Detay</a>";
		}
